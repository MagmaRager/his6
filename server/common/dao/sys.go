...
func NewLogin(c iris.Context) *sysDao {
	return &sysDao{ctx:c}
}
...
func (dao *sysDao) Sysdate() (time.Time, error) {
	...
	err := database.OraDb.Find(dao.ctx, &sysdate, sql)
	if err != nil {
...
func (dao *sysDao) GetBranch(branchId int) (model.CdBranch, error) {
	...
	err := database.OraDb.Find(dao.ctx, &branch, sql, branchId)
	if err != nil {
...
func (dao *sysDao) GetEmpid(empCode string) (int, error) {
	...
	err := database.OraDb.Find(dao.ctx, &id, sql, empCode)
	if err != nil {
...
func (dao *sysDao) GetSystemEmp(empId int) ([]model.BdSystem, error) {
	...
	err := database.OraDb.Query(dao.ctx, &systems, sql, empId)
	if err != nil {
...
func (dao *sysDao) GetParam(branchId int, parmName string) (database.NullableString, error) {
	...
	err := database.OraDb.Find(dao.ctx, &paramVal, sql, branchId, parmName)
	if err != nil {
...
func (dao *sysDao) SetParam(branchId int, parmName, value string) (int, error) {
	...
	rst, err := tx.Exec(dao.ctx, sql, value, branchId, parmName)
	if err != nil {
...
func (dao *sysDao) AddParam(branchId int, parmName, value string) error {
	...
	tx, _ := database.OraDb.BeginTx()
	_, err := tx.Exec(dao.ctx, sql, branchId, parmName, value)
	if err != nil {
		err = errors.New("设置菜单错误。" + err.Error())
		tx.Rollback()
		return err
...
func (dao *sysDao) GetParamEmp(empId int, parmName string) (database.NullableString, error) {
	...
	err := database.OraDb.Find(dao.ctx, &paramVal, sql, empId, parmName)
	if err != nil {
		err = errors.New("无对应参数值。" + err.Error())
		return paramVal, err
	}
...
func (dao *sysDao) SetParamEmp(empId int, parmName, value string) (int, error) {
	...
	tx, _ := database.OraDb.BeginTx()
	rst, err := tx.Exec(dao.ctx, sql, value, empId, parmName)
	if err != nil {
		err = errors.New("设置菜单错误。" + err.Error())
		tx.Rollback()
		return -1, err
...
func (dao *sysDao) AddParamEmp(empId int, parmName, value string) error {
	...
	tx, _ := database.OraDb.BeginTx()
	_, err := tx.Exec(dao.ctx, sql, empId, parmName, value)
	if err != nil {
		err = errors.New("设置菜单错误。" + err.Error())
		tx.Rollback()
		return err
...
func (dao *sysDao) Login(empCode, password string) (int, error) {
	...
	err := database.OraDb.Find(dao.ctx, &info, sql, empCode)
	if err != nil {
...
func (dao *sysDao) GetEmpInfo(empId int) (model.EmpInfo, error) {
	...
	err := database.OraDb.Find(dao.ctx, &empInfo, sql, empId)
	if err != nil {
...
func (dao *sysDao) GetRoleEmp(empId int) ([]string, error) {
	...
	err := database.OraDb.Query(dao.ctx, &roles, sql, empId)
	if err != nil {
...
func (dao *sysDao) GetAction(empId int) ([]model.Action, error) {
	...
	err := database.OraDb.Query(dao.ctx, &actions, sql, empId)
	if err != nil {
...
func (dao *sysDao) GetMenuEmp(empId, systemID int, roles string) ([]model.DataMenu, error) {
	...
	err := database.OraDb.Query(dao.ctx, &menus, sql, systemID, empId, systemID, roles)
	if err != nil {
